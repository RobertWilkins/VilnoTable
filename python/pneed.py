...
## This version of pneed.py has changes from Oct/Nov 2020, for external 
##   stat-proc connector incorporated , copied over (Jan 30 2021) from the 
##   temp typing directory (for ext. connector, Oct/Nov 2020).
## However, this file will also (soon) have to absorb a lot of changes from
##   Jan 2021, for the hodgepodge/finetune stuff
...
from bull import AVAILABLEVNAMES, THINGDEFNS, INFLAG_RESTRICTS, ONLYIFstack, \
   ...
   DENOMglobal, SUBPOPnull, SUBPOPglobal, H_for_GST, \
...
from mo_external import gssMexternal , gssOexternal
...
### Feb 2021 - ROOFcr is not needed at file scope, apparently it never was
...
## Feb 2021 - more input arguments for PROCESS_THIS_LEAF()
def PROCESS_THIS_LEAF(p,ROOFcr,ROOFcr_phys,   \
                       slot_ctr,num_slots,slot_fmt,digit_vec) :
  global gssMcrap
  global gssOcrap
  ### global ROOFcr   Feb 2021 - do not set as global anymore
  ...
  ## Feb 2021 - for PRINT-STAT feature, crossref_statistic_ROOF() 
  ##            called in pneed_hp.py, no longer call it here
  ...
  ## here, changes in November 2020, for ext. stat-proc connect upgrade  
  if MSPEC!=None :
   if MSPECsubpop!=None :
     SUBPOP1=MSPECsubpop 
   ## this is new, Nov 2020 
   elif gssM.__class__ == gssMexternal :
     SUBPOP1=SUBPOPnull()
   else :
  ...
  ## here, changes in November 2020, for ext. stat-proc connect upgrade  
  ## note, assuming thisrow? feature not used with ext. stat proc feature
  if MSPEC!=None :
    gssM.mo_clarify(MSPEC)   ## Sept 28 mo_clarify not mo_clarify1 
    gssO.mo_clarify1(OSPEC)
    gssM.bylist = set(SUBPOP_CAT)
    ## here is change, Nov 2020: sortby statement OVERRIDES subpop logic
    if gssM.__class__ == gssMexternal and gssM.sortby_ext != None :
      gssM.bylist = set(gssM.sortby_ext)
  ...
  ## Feb 2021 list of input arguments changes when calling prepGST_FIV()  
  prepGST_FIV(ROOFcr_phys,OUTPUTCOL_ROOFINDEX,statcolname,Hanal,OUTPUTCOL_CONST,  \
               slot_ctr,num_slots,slot_fmt,digit_vec)
...
#### move crossref_statistic_ROOF() to pneed_hp.py
